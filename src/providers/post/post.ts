...
export class PostProvider {
  ...
  async homePosts() {
    const data: any = await this.http
      .get(`${API_URL}/api/v1/home`, { headers: this.auth.authHeader() })
      .toPromise();
  ...
  private preparePostObject(post, data) {
    post.attributes["user"] = this.includeUser(
      post.relationships.user.data,
      data.included
    );
    post.attributes["hashtags"] = this.includeHashtags(
      post.relationships.hashtags.data,
      data.included
    );
  ...
  private includeHashtags(post_hashtags: any, included_data: any) {
    let hashtags_ids = post_hashtags.map(hashtags => {
      return hashtags.id;
    });
    return included_data
      .filter(included => {
        return (
          included.type == "hashtag" &&
          this.isThereHashtagId(hashtags_ids, included.id)
        );
      })
      .map(hashtag => {
